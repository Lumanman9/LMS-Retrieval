...
class DSE(nn.Module):
    def __init__(self, model_name="checkpoint/dse-phi3-v1", lora_adapter=None, bs=4, flash_attn=True):
        ...
        if not os.path.exists(model_name) or not os.path.isdir(model_name):
            # Determine original model repository based on the checkpoint path
            if "dse-phi3-docmatix-v2" in model_name:
                model_name = "Tevatron/dse-phi3-docmatix-v2"
            elif "dse-phi3-v1" in model_name:
                model_name = "Tevatron/dse-phi3-v1.0"
            print(f"[vision_wrapper.py - init] Local model path '{original_model_name}' not found, downloading from HuggingFace '{model_name}' (same as MMDocIR version)")

        def set_attention_implementation(model_name, flash_attn):
            # Only modify config if it's a local path
        ...
        set_attention_implementation(original_model_name, flash_attn)
...
class ColPaliRetriever():
    def __init__(self, bs=4, use_gpu=True):
        self.bs = bs
        self.bs_query = 32
        model_name = "checkpoint/colpali-v1.1"
        base_ckpt = "checkpoint/colpaligemma-3b-mix-448-base"
        
        # If local paths don't exist, fall back to original HuggingFace models (cloned by MMDocIR)
        if not os.path.exists(model_name) or not os.path.isdir(model_name):
            print(f"[vision_wrapper.py - init] Local adapter path '{model_name}' not found, downloading from HuggingFace 'vidore/colpali-v1.1' (same as MMDocIR version)")
            self.model_name = "vidore/colpali-v1.1"
        else:
            self.model_name = model_name
            
        if not os.path.exists(base_ckpt) or not os.path.isdir(base_ckpt):
            print(f"[vision_wrapper.py - init] Local base checkpoint path '{base_ckpt}' not found, downloading from HuggingFace 'vidore/colpaligemma-3b-mix-448-base' (same as MMDocIR version)")
            self.base_ckpt = "vidore/colpaligemma-3b-mix-448-base"
        else:
...
class ColQwen2Retriever:
    def __init__(self, bs=4, use_gpu=True):
        self.bs = bs
        self.bs_query = 64
        model_name = "checkpoint/colqwen2-v1.0"
        base_ckpt = "checkpoint/colqwen2-base"
        
        # If local paths don't exist, fall back to original HuggingFace models (cloned by MMDocIR)
        if not os.path.exists(model_name) or not os.path.isdir(model_name):
            print(f"[vision_wrapper.py - init] Local adapter path '{model_name}' not found, downloading from HuggingFace 'vidore/colqwen2-v1.0' (same as MMDocIR version)")
            self.model_name = "vidore/colqwen2-v1.0"
        else:
            self.model_name = model_name
            
        if not os.path.exists(base_ckpt) or not os.path.isdir(base_ckpt):
            print(f"[vision_wrapper.py - init] Local base checkpoint path '{base_ckpt}' not found, downloading from HuggingFace 'vidore/colqwen2-base' (same as MMDocIR version)")
            self.base_ckpt = "vidore/colqwen2-base"
        else:
            self.base_ckpt = base_ckpt
