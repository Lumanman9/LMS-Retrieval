...
class BGE(Sent_Retriever):
    def __init__(self, bs=256, use_gpu=True, model_path="checkpoint/bge-large-en-v1.5"):
        ...
        # If local path doesn't exist, fall back to original HuggingFace model (cloned by MMDocIR)
        # Note: MMDocIR models are in subdirectories, so we use the original repository for auto-download
        if not os.path.exists(model_path) or not os.path.isdir(model_path):
            print(f"[text_wrapper.py - init] Local model path '{model_path}' not found, downloading from HuggingFace 'BAAI/bge-large-en-v1.5' (same as MMDocIR version)")
...
class E5(Sent_Retriever):
    def __init__(self, bs=256, use_gpu=True, model_path="checkpoint/e5-large-v2"):
        ...
        # If local path doesn't exist, fall back to original HuggingFace model (cloned by MMDocIR)
        if not os.path.exists(model_path) or not os.path.isdir(model_path):
            print(f"[text_wrapper.py - init] Local model path '{model_path}' not found, downloading from HuggingFace 'intfloat/e5-large-v2' (same as MMDocIR version)")
            self.model_path = "intfloat/e5-large-v2"
        else:
            self.model_path = model_path
        self.model = SentenceTransformer(self.model_path)
...
class GTE(Sent_Retriever):
    def __init__(self, bs=256, use_gpu=True, model_path="checkpoint/gte-large"):
        ...
        # If local path doesn't exist, fall back to original HuggingFace model (cloned by MMDocIR)
        if not os.path.exists(model_path) or not os.path.isdir(model_path):
            print(f"[text_wrapper.py - init] Local model path '{model_path}' not found, downloading from HuggingFace 'thenlper/gte-large' (same as MMDocIR version)")
            self.model_path = "thenlper/gte-large"
        else:
            self.model_path = model_path
        self.model = SentenceTransformer(self.model_path)
...
class Contriever():
    def __init__(self, bs = 256, use_gpu= True, model_path='checkpoint/contriever-msmarco'):
        ...
        # If local path doesn't exist, fall back to original HuggingFace model (cloned by MMDocIR)
        if not os.path.exists(model_path) or not os.path.isdir(model_path):
            print(f"[text_wrapper.py - init] Local model path '{model_path}' not found, downloading from HuggingFace 'facebook/contriever-msmarco' (same as MMDocIR version)")
            self.model_path = "facebook/contriever-msmarco"
        else:
            self.model_path = model_path
...
class DPR():
    def __init__(self, bs = 256, use_gpu=True, model_path="checkpoint"):
        ...
        self.model_path = model_path
        # Check if local paths exist, otherwise use HuggingFace model identifiers
        question_encoder_path = model_path + "/dpr-question_encoder-multiset-base"
        ctx_encoder_path = model_path + "/dpr-ctx_encoder-multiset-base"
        
        if not os.path.exists(question_encoder_path) or not os.path.isdir(question_encoder_path):
            print(f"[text_wrapper.py - init] Local question encoder path '{question_encoder_path}' not found, downloading from HuggingFace 'facebook/dpr-question_encoder-multiset-base' (same as MMDocIR version)")
            question_encoder_path = "facebook/dpr-question_encoder-multiset-base"
        if not os.path.exists(ctx_encoder_path) or not os.path.isdir(ctx_encoder_path):
            print(f"[text_wrapper.py - init] Local context encoder path '{ctx_encoder_path}' not found, downloading from HuggingFace 'facebook/dpr-ctx_encoder-multiset-base' (same as MMDocIR version)")
            ctx_encoder_path = "facebook/dpr-ctx_encoder-multiset-base"
        self.query_tok = DPRQuestionEncoderTokenizer.from_pretrained(question_encoder_path)
        self.query_enc = DPRQuestionEncoder.from_pretrained(question_encoder_path)
        self.ctx_tok = DPRContextEncoderTokenizer.from_pretrained(ctx_encoder_path)
        self.ctx_enc = DPRContextEncoder.from_pretrained(ctx_encoder_path)
        self.bs = bs
...
class ColBERTReranker:
    def __init__(self, bs = 256, use_gpu= True, model_path="checkpoint/colbertv2.0"):
        ...
        # If local path doesn't exist, fall back to original HuggingFace model (cloned by MMDocIR)
        if not os.path.exists(model_path) or not os.path.isdir(model_path):
            print(f"[text_wrapper.py - init] Local model path '{model_path}' not found, downloading from HuggingFace 'colbert-ir/colbertv2.0' (same as MMDocIR version)")
            self.model_path = "colbert-ir/colbertv2.0"
        else:
            self.model_path = model_path
        self.bs = bs
